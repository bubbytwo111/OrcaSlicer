...
wxBoxSizer* PreferencesDialog::create_item_link_association( wxString url_prefix, wxString website_name)
{
    ...
    wxString tooltip = _L("Associate") + " " + url_prefix + ":// " + _L("with ROCK3TS 3D so models from") + " " + website_name + " " + _L("open in this app");
...
void PreferencesDialog::create_items()
{
    ...
    auto item_single_instance  = create_item_checkbox(_L("Use a single ROCK3TS 3D window"),
    #if __APPLE__
    ...
    #else
            _L("If this is enabled, opening ROCK3TS 3D again will bring the current window forward instead of opening another copy."),
    #endif
            "single_instance");
    ...
    auto item_auto_reslice = create_item_auto_reslice(
        ...
        _L("If enabled, ROCK3TS 3D will re-slice automatically whenever slicing-related settings change."),
    ...
    g_sizer->Add(create_item_title(_L("Open files with ROCK3TS 3D")), 1, wxEXPAND);

    auto item_associate_3mf    = create_item_checkbox(_L("Open 3MF files with ROCK3TS 3D"), _L("If enabled, sets ROCK3TS 3D as the default app for 3MF files.") , "associate_3mf");
    g_sizer->Add(item_associate_3mf);

    auto item_associate_drc = create_item_checkbox(_L("Open DRC files with ROCK3TS 3D"), _L("If enabled, sets ROCK3TS 3D as the default app for DRC files."), "associate_drc");
    g_sizer->Add(item_associate_drc);

    auto item_associate_stl    = create_item_checkbox(_L("Open STL files with ROCK3TS 3D"), _L("If enabled, sets ROCK3TS 3D as the default app for STL files.") , "associate_stl");
    g_sizer->Add(item_associate_stl);

    auto item_associate_step   = create_item_checkbox(_L("Open STEP files with ROCK3TS 3D"), _L("If enabled, sets ROCK3TS 3D as the default app for STEP files."), "associate_step");
    g_sizer->Add(item_associate_step);

    //// ASSOCIATE > WebLinks
    g_sizer->Add(create_item_title(_L("Open web links with ROCK3TS 3D")), 1, wxEXPAND);
